...
public class LinearLayoutCompat extends ViewGroup {
    ...
    void measureHorizontal(int i, int i2) {
        ...
        if (iArr[VERTICAL] == -1 && iArr[SHOW_DIVIDER_NONE] == -1 && iArr[SHOW_DIVIDER_MIDDLE] == -1 && iArr[INDEX_FILL] == -1) {
            ...
            i10 = Math.max(i5, Math.max(iArr[INDEX_FILL], Math.max(iArr[SHOW_DIVIDER_NONE], Math.max(iArr[VERTICAL], iArr[SHOW_DIVIDER_MIDDLE]))) + Math.max(iArr2[INDEX_FILL], Math.max(iArr2[SHOW_DIVIDER_NONE], Math.max(iArr2[VERTICAL], iArr2[SHOW_DIVIDER_MIDDLE]))));
        }
