    ...
    public ASN1Primitive d() throws IOException {
        ...
        if (read > 0) {
            ...
            if (b >= 0) {
                ...
                if ((read & 64) != 0) {
                    return new BERApplicationSpecificParser(a, aSN1StreamParser).h();
                }
                if ((read & 128) != 0) {
                    return new BERTaggedObjectParser(true, a, aSN1StreamParser).h();
                }
                switch (a) {
                    case p.View_theme /*4*/:
                        return new BEROctetStringParser(aSN1StreamParser).h();
                    case p.Toolbar_contentInsetRight /*8*/:
                        return new DERExternalParser(aSN1StreamParser).h();
                    case p.Toolbar_titleMarginBottom /*16*/:
                        return new BERSequenceParser(aSN1StreamParser).h();
                    case p.Toolbar_maxButtonHeight /*17*/:
                        return new BERSetParser(aSN1StreamParser).h();
                    default:
