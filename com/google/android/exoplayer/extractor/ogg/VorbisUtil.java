...
final class VorbisUtil {
    ...
    public static boolean verifyVorbisHeaderCapturePattern(int i, ParsableByteArray parsableByteArray, boolean z) throws ParserException {
        ...
        } else if (parsableByteArray.readUnsignedByte() == 118 && parsableByteArray.readUnsignedByte() == bdd.AppCompatTheme_switchStyle && parsableByteArray.readUnsignedByte() == 114 && parsableByteArray.readUnsignedByte() == 98 && parsableByteArray.readUnsignedByte() == bdd.AppCompatTheme_radioButtonStyle && parsableByteArray.readUnsignedByte() == 115) {
            return true;
        } else {
            if (z) {
                return false;
        ...
        for (int i2 = 0; i2 < readBits; i2++) {
            ...
            switch (readBits2) {
                case Util.TYPE_DASH /*0*/:
                    int i3;
                    if (vorbisBitArray.readBit()) {
                        readBits2 = vorbisBitArray.readBits(4) + 1;
                    } else {
                        readBits2 = 1;
                    }
                    if (vorbisBitArray.readBit()) {
    ...
    private static void readResidues(VorbisBitArray vorbisBitArray) throws ParserException {
        ...
        for (int i = 0; i < readBits; i++) {
            ...
            vorbisBitArray.skipBits(24);
            vorbisBitArray.skipBits(24);
            int readBits3 = vorbisBitArray.readBits(6) + 1;
            vorbisBitArray.skipBits(8);
            int[] iArr = new int[readBits3];
            for (i2 = 0; i2 < readBits3; i2++) {
                int readBits4 = vorbisBitArray.readBits(3);
                if (vorbisBitArray.readBit()) {
                    readBits2 = vorbisBitArray.readBits(5);
                } else {
                    readBits2 = 0;
                }
                iArr[i2] = (readBits2 * 8) + readBits4;
            }
            for (i2 = 0; i2 < readBits3; i2++) {
                for (readBits2 = 0; readBits2 < 8; readBits2++) {
                    if ((iArr[i2] & (1 << readBits2)) != 0) {
                        vorbisBitArray.skipBits(8);
                    }
                }
            }
